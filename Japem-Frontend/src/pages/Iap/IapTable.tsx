...
export const IapTable = ({ userRole }: IapTableProps) => {
  // Lógica de roles: Admin y Asistencial pueden editar. Donativos solo lee.
  const role = (userRole || "").toLowerCase();
  const allowedRoles = ["admin", "superadmin", "editor", "asistencial"];
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      Swal.fire(
        "¡Importación Exitosa!",
        response.data.message || "Datos cargados.",
        "success",
      );
      fetchIaps();
    } catch (error: any) {
  ...
  const filteredIaps = iaps.filter(
    (iap) =>
      ...
      <div className="relative flex items-center justify-center mb-8">
        <div className="text-center">
          ...
          <p className="text-[#817e7e] mt-1">
            Directorio de instituciones y análisis de necesidades.
          </p>
        </div>
        ...
        {!isReadOnly && (
          <div className="absolute right-0 top-1/2 transform -translate-y-1/2 flex items-center gap-3">
            ...
            <button
              ...
              className="cursor-pointer group bg-[#719c44] hover:bg-[#5e8239] text-white px-5 py-2.5 rounded-xl flex items-center gap-2 shadow-md hover:shadow-xl font-bold transition-all active:scale-95"
            >
              <Plus size={20} />{" "}
                ...
                return (
                  <div>
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      <span className="text-[10px] bg-[#f2f5f0] text-[#719c44] px-1.5 py-0.5 rounded border border-[#c0c6b6] font-bold uppercase tracking-wide">
                        {row.rubro}
                      </span>
                      <span
                        className={`text-[10px] font-bold px-1.5 py-0.5 rounded ${row.estatus === "Activa" ? "bg-green-50 text-green-700" : "bg-red-50 text-red-700"}`}
                      >
                        {row.estatus}
                      </span>
                    </div>
                ...
                return (
                  <div className="flex flex-wrap gap-1 max-w-[150px]">
                    {items.map((item, idx) => (
                      <span
                        key={idx}
                        className={`px-1.5 py-0.5 rounded text-[10px] font-bold border ${getClasifColor(item.trim())}`}
                      >
                        {item.trim()}
                      </span>
                    ))}
              ...
              if (key === "es_certificada") {
                return (
                  <div className="flex gap-2 justify-start">
                    <Award
                      size={18}
                      className={`transition-transform ${row.es_certificada ? "text-yellow-500" : "text-[#c0c6b6]"}`}
                    />
                    <FileCheck
                      size={18}
                      className={`transition-transform ${row.tiene_donataria_autorizada ? "text-blue-500" : "text-[#c0c6b6]"}`}
                    />
                    <CheckCircle
                      size={18}
                      className={`transition-transform ${row.tiene_padron_beneficiarios ? "text-[#719c44]" : "text-[#c0c6b6]"}`}
                    />
                  </div>
                );
              }
              if (key === "veces_donado") {
                return (
                  <div className="flex justify-start">
                    <span className="text-xs font-bold text-[#817e7e] flex items-center gap-1">
                      <History size={12} /> {value}
                    </span>
                  </div>
                );
              }
                ...
                return (
                  <div className="flex gap-2 justify-start">
                    <button
                      ...
                      className="cursor-pointer p-1.5 hover:bg-blue-50 text-[#817e7e] hover:text-blue-600 rounded transition hover:scale-110"
                      title="Ver Detalles"
                    >
                      <Eye size={16} />
                    </button>
                    {!isReadOnly && (
                      <>
                        <button
                          ...
                          className="cursor-pointer p-1.5 hover:bg-[#f2f5f0] text-[#817e7e] hover:text-[#719c44] rounded transition hover:scale-110"
                        >
                          <Edit size={16} />
                        </button>
                        <button
                          onClick={() => row.id && handleDelete(row.id)}
                          className="cursor-pointer p-1.5 hover:bg-red-50 text-[#817e7e] hover:text-red-500 rounded transition hover:scale-110"
                        >
                          <Trash2 size={16} />
                        </button>
                      </>
          ...
          <form onSubmit={handleSave} className="space-y-6 p-1">
            ...
            <div className="p-5 bg-[#f2f5f0] rounded-xl border border-[#c0c6b6]">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div className="md:col-span-2">
                  <label className="text-xs font-bold text-[#353131] block mb-1">
                    Nombre de la IAP *
                  </label>
                  <input
                    type="text"
                    required
                    className="w-full px-4 py-2.5 border border-[#c0c6b6] rounded-lg text-sm uppercase focus:ring-4 focus:ring-[#719c44]/20 focus:border-[#719c44] outline-none text-[#353131]"
                    value={form.nombre_iap}
                    onChange={(e) =>
                      setForm({ ...form, nombre_iap: e.target.value })
                    }
                  />
                </div>
                <div>
                  ...
                  <select
                    className="w-full px-4 py-2.5 border border-[#c0c6b6] rounded-lg text-sm bg-white"
                    value={form.estatus || ""}
                    onChange={(e) =>
                ...
                <div>
                  <label className="text-xs font-bold text-[#353131] block mb-2">
                  ...
                  <div className="grid grid-cols-4 gap-2 bg-white p-3 border border-[#c0c6b6] rounded-xl max-h-32 overflow-y-auto custom-scrollbar">
                    {listaClasificaciones.map((clasif) => (
                      <label
                        key={clasif}
                        className="flex items-center gap-1.5 cursor-pointer hover:bg-gray-50 rounded p-1"
                      >
                        <input
                          type="checkbox"
                          className="accent-[#719c44] w-4 h-4 cursor-pointer"
                          checked={String(form.clasificacion || "")
                            .split(SEPARATOR)
                            .includes(clasif)}
                          onChange={() =>
                            toggleMultiSelect("clasificacion", clasif)
                          }
                        />
              ...
              <div className="grid grid-cols-1 gap-5">
                <div>
                  ...
                  >
                    ...
                    <option value="Niñas, Niños y Adolescentes">
                ...
                <div>
                  <label className="text-xs font-bold text-[#353131] block mb-2">
                    Actividades Específicas (Múltiple)
                  </label>
                  <div className="bg-white p-3 border border-[#c0c6b6] rounded-xl max-h-40 overflow-y-auto custom-scrollbar space-y-1">
                    {listaActividades.map((act, i) => (
                      <label
                        key={i}
                        className="flex items-start gap-2 cursor-pointer hover:bg-gray-50 p-1.5 rounded transition-colors border-b border-gray-50 last:border-0"
                      >
                        <input
                          type="checkbox"
                          className="accent-[#719c44] w-4 h-4 mt-0.5 cursor-pointer flex-shrink-0"
                          checked={String(form.actividad_asistencial || "")
                            .split(SEPARATOR)
                            .includes(act)}
                          onChange={() =>
                            toggleMultiSelect("actividad_asistencial", act)
                          }
                        />
                        <span className="text-xs text-[#555] leading-tight select-none">
                          {act}
                        </span>
                      </label>
                    ))}
            ...
            <div className="p-5 bg-[#f2f5f0] rounded-xl border border-[#c0c6b6]">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-5">
                <div>
                  ...
                  <input
                    ...
                    min="0"
                    placeholder="0"
                    className="w-full px-3 py-2 border border-[#c0c6b6] rounded-lg text-sm bg-white focus:border-[#719c44] outline-none"
                    value={cantFijos}
                    onChange={(e) => setCantFijos(e.target.value)}
                  />
                </div>
                <div>
                  <label className="text-xs font-bold text-[#353131] block mb-1">
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2 border border-[#c0c6b6] rounded-lg text-sm bg-white focus:border-[#719c44] outline-none"
                    value={cantTemp}
                    onChange={(e) => setCantTemp(e.target.value)}
                  />
                </div>
                <div>
                  <label className="text-xs font-bold text-[#353131] block mb-1">
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2 border border-[#c0c6b6] rounded-lg text-sm bg-white focus:border-[#719c44] outline-none"
                    value={cantFlot}
                    onChange={(e) => setCantFlot(e.target.value)}
                  />
                </div>
              </div>
              ...
              <div>
                ...
                <input
                  type="text"
                  className="w-full px-4 py-2.5 border border-[#c0c6b6] rounded-lg text-sm uppercase focus:ring-4 focus:ring-[#719c44]/20 focus:border-[#719c44] outline-none transition-all text-[#353131]"
                  placeholder="Ej. JUGUETES, ROPA..."
                  value={form.necesidad_complementaria || ""}
                  onChange={(e) =>
                    setForm({
                      ...form,
                      necesidad_complementaria: e.target.value,
                    })
                  }
                />
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-2">
              <label className="flex items-center gap-3 cursor-pointer p-4 border-2 border-transparent bg-white rounded-xl shadow-sm group has-[:checked]:border-[#719c44] has-[:checked]:bg-[#f2f5f0]">
                ...
                <div className="flex-1">
                  <span className="font-bold text-[#353131] text-sm">
                    Certificada
                  </span>
                </div>
                <input
                  type="checkbox"
                  className="w-5 h-5 accent-[#719c44]"
                  checked={form.es_certificada}
                  onChange={(e) =>
                    setForm({ ...form, es_certificada: e.target.checked })
                  }
                />
              </label>
              <label className="flex items-center gap-3 cursor-pointer p-4 border-2 border-transparent bg-white rounded-xl shadow-sm group has-[:checked]:border-[#719c44] has-[:checked]:bg-[#f2f5f0]">
                ...
                <input
                  type="checkbox"
                  className="w-5 h-5 accent-[#719c44]"
                  checked={form.tiene_donataria_autorizada}
                  onChange={(e) =>
                    setForm({
                      ...form,
                      tiene_donataria_autorizada: e.target.checked,
                    })
                  }
                />
              </label>
              <label className="flex items-center gap-3 cursor-pointer p-4 border-2 border-transparent bg-white rounded-xl shadow-sm group has-[:checked]:border-[#719c44] has-[:checked]:bg-[#f2f5f0]">
                ...
                <input
                  type="checkbox"
                  className="w-5 h-5 accent-[#719c44]"
                  checked={form.tiene_padron_beneficiarios}
                  onChange={(e) =>
                    setForm({
                      ...form,
                      tiene_padron_beneficiarios: e.target.checked,
                    })
                  }
                />
              </label>
            ...
            <div className="flex justify-end gap-3 border-t pt-6 border-[#c0c6b6]/30">
              <button
                ...
                className="cursor-pointer px-6 py-3 text-[#817e7e] font-bold hover:bg-[#f2f5f0] rounded-xl"
              >
                Cancelar
              </button>
              <button
                type="submit"
                className="cursor-pointer px-8 py-3 bg-[#719c44] hover:bg-[#5e8239] text-white font-bold rounded-xl shadow-lg flex items-center gap-2"
              >
        ...
        {selectedIap && (
          <div className="space-y-6 p-2">
            <div className="text-center border-b border-[#f2f5f0] pb-6">
              <div className="inline-block p-4 bg-[#f2f5f0] rounded-full mb-3 border-4 border-white shadow-sm">
                <Building2 size={40} className="text-[#719c44]" />
              </div>
              <h2 className="text-xl font-bold text-[#353131] uppercase">
                {selectedIap.nombre_iap}
              </h2>
              <span className="px-3 py-1 bg-gray-100 text-gray-600 rounded-full text-xs font-bold uppercase mt-2 inline-block">
                {selectedIap.rubro}
              </span>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="bg-white p-5 rounded-2xl border border-[#e5e7eb] shadow-sm">
                <p className="text-xs font-bold text-[#817e7e] uppercase mb-2">
                ...
                <div className="flex flex-wrap gap-1 mb-3">
                  {selectedIap.clasificacion?.split(SEPARATOR).map((c, i) => (
                    <span
                      key={i}
                      className={`px-2 py-0.5 text-xs font-bold border rounded ${getClasifColor(c.trim())}`}
                    >
                      {c.trim()}
                    </span>
                  ))}
                ...
                <ul className="text-sm text-[#353131] list-disc list-inside space-y-1">
                  {selectedIap.actividad_asistencial
                    ?.split(SEPARATOR)
                    .map((a, i) => (
              ...
              <div className="bg-white p-5 rounded-2xl border border-[#e5e7eb] shadow-sm">
                ...
                <span className="text-3xl font-extrabold text-[#353131] block mb-2">
                  {selectedIap.personas_beneficiadas} Total
                </span>
                <div className="flex flex-col gap-1">
                  {selectedIap.tipo_beneficiario
                    ?.split(SEPARATOR)
                    .map((t, i) => (
                      <span
                        key={i}
                        className="bg-[#f2f5f0] text-[#719c44] px-3 py-1 rounded text-xs font-bold border border-[#c0c6b6]"
                      >
                        {t.trim()}
                      </span>
                    ))}
            ...
            <div className="bg-[#f9fafb] p-5 rounded-2xl border border-[#e5e7eb]">
              <h4 className="text-sm font-bold text-[#353131] flex items-center gap-2 mb-3">
                <Heart size={16} className="text-[#719c44]" /> Necesidades
                Especiales
              </h4>
              <p className="text-sm text-[#555] bg-white p-3 rounded-xl border border-[#e5e7eb]">
                {selectedIap.necesidad_complementaria || "Sin registro."}
              </p>
            </div>
            <div className="pt-2">
              <button
                onClick={() => setIsViewModalOpen(false)}
                className="cursor-pointer w-full py-3 bg-[#353131] hover:bg-black text-white font-bold rounded-xl"
              >
                Cerrar Ficha
              </button>
            </div>
