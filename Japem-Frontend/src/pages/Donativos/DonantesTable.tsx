...
export default function DonantesTable({ userRole }: DonantesTableProps) {
  ...
  // Referencia para el input de archivo CSV
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const API_URL =
        import.meta.env.VITE_API_URL || "http://192.168.1.198:8000/api";

      const response = await axios.post(
        `${API_URL}/donantes/importar`,
        formDataFile,
        {
          headers: {
      ...
      Swal.fire(
        "¡Importación Exitosa!",
        response.data.message || "Donantes cargados.",
        "success",
      );
      fetchDonantes();
    } catch (error: any) {
      ...
      <div className="relative flex items-center justify-center mb-8">
        <div className="text-center">
          ...
          <p className="text-[#817e7e] mt-1">
            Gestiona la información de empresas y particulares
          </p>
        </div>

        {/* 5. CONDICIONAL: OCULTAR BOTÓN "NUEVO" E "IMPORTAR" SI ES LECTOR */}
        {!isReadOnly && (
              ...
              if (key === "estatus") {
                return (
                  <span
                    ...
                    ${
                      value === "Permanente"
                        ? "bg-[#f2f5f0] text-[#719c44]"
                        : value === "Eventual"
                          ? "bg-[#ffedcc] text-[#d97706]"
                ...
                return (
                  <div className="flex gap-2 justify-center">
                    <button
                      ...
                      className="cursor-pointer p-1.5 text-[#817e7e] hover:bg-[#f2f5f0] hover:text-[#719c44] rounded-md transition transform hover:scale-110"
                      title="Editar"
                    >
                      <Edit size={18} />
                    </button>
                    <button
                      ...
                      className="cursor-pointer p-1.5 text-[#817e7e] hover:bg-red-50 hover:text-red-500 rounded-md transition transform hover:scale-110"
                      title="Eliminar"
                    >
                      <Trash2 size={18} />
                    </button>
          ...
          <form onSubmit={handleSave} className="space-y-6 p-1">
            ...
            <div className="p-5 bg-[#f2f5f0] rounded-xl border border-[#c0c6b6]">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                  <label className="block text-xs font-bold text-[#353131] mb-1">
                    Razón Social / Nombre *
                  </label>
                  <input
                    type="text"
                    required
                    className="w-full px-4 py-2.5 border border-[#c0c6b6] rounded-lg text-sm bg-white focus:ring-4 focus:ring-[#719c44]/20 focus:border-[#719c44] outline-none transition-all uppercase text-[#353131]"
              ...
              <div className="mt-4">
                ...
                <div className="relative">
                  <MapPin
                    className="absolute top-3 left-3 text-[#817e7e]"
                    size={16}
                  />
                  <textarea
                    className="w-full pl-10 pr-4 py-2.5 border border-[#c0c6b6] rounded-lg text-sm bg-white focus:ring-4 focus:ring-[#719c44]/20 focus:border-[#719c44] outline-none transition-all uppercase text-[#353131]"
                    rows={2}
            ...
            <div className="p-5 bg-[#f9fafb] rounded-xl border border-[#e5e7eb]">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mt-4">
                <div>
                  ...
                  <div className="relative">
                    <Mail
                      className="absolute top-1/2 -translate-y-1/2 left-3 text-[#817e7e]"
                      size={16}
                    />
                    <input
                      type="email"
                      className="w-full pl-10 pr-4 py-2.5 border border-[#c0c6b6] rounded-lg text-sm bg-white focus:ring-4 focus:ring-[#719c44]/20 focus:border-[#719c44] outline-none transition-all text-[#353131]"
            ...
            <div className="flex justify-end gap-3 mt-6 pt-4 border-t border-[#c0c6b6]/30">
              <button
                ...
                className="cursor-pointer px-6 py-3 text-[#817e7e] font-bold hover:bg-[#f2f5f0] rounded-xl transition-all"
              >
                Cancelar
              </button>
              <button
                type="submit"
                className="cursor-pointer px-8 py-3 bg-[#719c44] hover:bg-[#5e8239] text-white font-bold rounded-xl shadow-lg shadow-[#719c44]/30 transition-all transform active:scale-95"
              >
