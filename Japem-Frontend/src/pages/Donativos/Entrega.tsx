...
export const Entrega = ({ userRole }: EntregaProps) => {
  ...
  const [selectedEntrega, setSelectedEntrega] = useState<EntregaItem | null>(
    null,
  );
  ...
  const handleConfirmar = async (e: React.FormEvent) => {
    e.preventDefault();
    if (isReadOnly)
      return Swal.fire("Acceso Denegado", "No tienes permiso.", "error");
    ...
    try {
      ...
      Swal.fire(
        "¡Entrega Registrada!",
        "Salida confirmada correctamente.",
        "success",
      );
    ...
    <div className="p-6 animate-fade-in relative w-full max-w-full">
      <div className="relative flex items-center justify-center mb-8">
        <div className="text-center">
          <h1 className="text-2xl font-bold text-[#353131] flex items-center justify-center gap-2">
            <Truck className="text-[#719c44]" size={28} /> Mesa de Control de
            Entregas
          </h1>
          <p className="text-[#817e7e] mt-1">
            Gestiona las salidas de almacén pendientes y el historial.
          </p>
        </div>
      </div>
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center py-20 bg-white rounded-2xl shadow-sm border border-[#c0c6b6]/30 animate-pulse">
          ...
          <p className="text-[#817e7e] font-medium">
            Cargando mesa de control...
          </p>
        </div>
      ) : (
        <div className="bg-white rounded-2xl shadow-xl shadow-[#c0c6b6]/20 border border-[#c0c6b6]/30 overflow-hidden">
          <Table
            ...
            renderCell={(key, value, row) => {
              const isEntregado =
                row.estatus === "procesado" || row.estatus === "entregado";
                  ...
                  <div className="flex flex-col">
                    ...
                    {isEntregado && row.fecha_entrega_real && (
                      <span className="text-[10px] text-[#719c44] font-bold mt-1 pl-6">
                        Entregado:{" "}
                        {new Date(row.fecha_entrega_real).toLocaleDateString(
                          "es-MX",
                        )}
                      </span>
                    )}
              ...
              if (key === "nombre_iap")
                return (
                  <span className="font-bold text-[#353131]">{value}</span>
                );
              ...
              if (key === "producto_nombre") {
                return (
                  <div className="flex items-center gap-2 text-[#817e7e]">
                    ...
                    <span className="bg-[#f2f5f0] text-[#719c44] text-xs px-2 py-0.5 rounded-full font-bold border border-[#c0c6b6]/50">
                      x{row.cantidad}
                    </span>
                  </div>
                );
            ...
            <form onSubmit={handleConfirmar} className="space-y-6 p-1">
              ...
              <div className="bg-[#f2f5f0] p-6 rounded-2xl border border-[#c0c6b6]">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    <p className="text-xs text-[#817e7e] uppercase font-bold">
                      Beneficiario
                    </p>
                    <p className="text-base font-bold text-[#353131]">
                      {selectedEntrega.nombre_iap}
                    </p>
                  </div>
                  <div>
                    <p className="text-xs text-[#817e7e] uppercase font-bold">
                    ...
                    <div className="flex items-center gap-2">
                      <span className="text-base font-bold text-[#353131]">
                        {selectedEntrega.producto_nombre}
                      </span>
                      <span className="bg-white text-[#719c44] px-2 py-0.5 rounded border border-[#c0c6b6] text-xs font-bold">
                        Cant: {selectedEntrega.cantidad}
                      </span>
                    </div>
                  </div>
                </div>
              ...
              <div className="grid grid-cols-1 gap-6">
                <div className="space-y-1.5">
                  <label className="text-sm font-bold text-[#353131] flex items-center gap-2">
                    <UserCheck size={16} className="text-[#817e7e]" />{" "}
                    Responsable JAPEM
                  </label>
                  <select
                    required
                    className="w-full p-3 border border-[#c0c6b6] rounded-xl outline-none text-[#353131] bg-white"
                    value={form.responsable_entrega}
                    onChange={(e) =>
                      setForm({ ...form, responsable_entrega: e.target.value })
                    }
                  >
                ...
                <div className="space-y-1.5">
                  <label className="text-sm font-bold text-[#353131] flex items-center gap-2">
                    <MapPin size={16} className="text-[#817e7e]" /> Lugar
                  </label>
                  <select
                    className="w-full p-3 border border-[#c0c6b6] rounded-xl outline-none bg-white text-[#353131]"
                    value={form.lugar_entrega}
                    onChange={(e) =>
                      setForm({ ...form, lugar_entrega: e.target.value })
                    }
                  >
                    <option value="Oficinas JAPEM">Oficinas JAPEM</option>
                    <option value="Instalaciones de la IAP">
                      Instalaciones de la IAP
                    </option>
              ...
              <div className="pt-4 border-t border-[#c0c6b6]/30 flex justify-end gap-3">
                ...
                <button
                  type="submit"
                  className="px-8 py-3 bg-[#719c44] hover:bg-[#5e8239] text-white font-bold rounded-xl shadow-lg flex items-center gap-2"
                >
        ...
        {selectedEntrega && (
          <div className="space-y-6 p-2">
            ...
            <div className="text-center mb-6">
              <h3 className="text-xl font-bold text-[#353131]">
                Entrega Exitosa
              </h3>
              <p className="text-[#817e7e] text-sm">
                Folio de Referencia: #{selectedEntrega.id}
              </p>
            </div>
            ...
            <div className="bg-white border border-[#c0c6b6]/50 rounded-2xl overflow-hidden shadow-sm">
              <div className="p-4 border-b border-[#f2f5f0] bg-[#f9fafb] flex justify-between items-center">
                <span className="text-xs font-bold text-[#817e7e] uppercase tracking-wider">
                  Fecha Real
                </span>
              ...
              <div className="p-5 space-y-4">
                <div className="flex items-start gap-4">
                  <div className="p-2 bg-[#f2f5f0] rounded-lg text-[#719c44]">
                    <UserCheck size={20} />
                  </div>
                  <div>
                    <p className="text-xs text-[#817e7e] font-bold uppercase">
                      Entregado Por
                    </p>
                    <p className="text-base font-medium text-[#353131]">
                      {selectedEntrega.responsable_entrega || "Sin dato"}
                    </p>
                  </div>
                </div>
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-base font-medium text-[#353131]">
                      {selectedEntrega.lugar_entrega || "Sin dato"}
                    </p>
                  </div>
                </div>
              </div>
