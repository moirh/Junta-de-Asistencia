...
export const EntregasView = ({ userRole }: EntregasViewProps) => {
  ...
  const loadInventario = async () => {
    try {
      ...
      if (Array.isArray(data)) {
        setInventario(
          data.filter((i: any) => (i.stock_actual ?? i.cantidad ?? 0) > 0),
        );
      } else {
        setInventario([]);
      }
  ...
  const groupedInventory = inventario.reduce(
    (groups, item) => {
  ...
  const handleSelectProducto = async (item: any) => {
    ...
    try {
      ...
      const response = await axios.get(
        `${API_URL}/entregas/sugerencias/${item.id}`,
        {
          headers: {
  ...
  const handleConfirmAsignacion = async () => {
    if (isReadOnly) {
      Swal.fire(
        "Acceso Denegado",
        "No tienes permisos para asignar entregas.",
        "error",
      );
      return;
    }
    ...
    const stockDisponible =
      selectedItem.stock_actual ?? selectedItem.cantidad ?? 0;

    if (cantidadFinal > stockDisponible) {
      Swal.fire({
        title: "Stock Insuficiente",
        text: `Solo tienes ${stockDisponible} unidades disponibles.`,
        icon: "warning",
        confirmButtonColor: "#719c44",
      });
      return;
      ...
      <div className="lg:col-span-4 bg-white p-4 rounded-2xl border border-[#c0c6b6]/30 shadow-sm flex flex-col h-[calc(100vh-140px)]">
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center flex-1 h-full animate-in fade-in">
            ...
            <p className="text-[#817e7e] font-medium animate-pulse">
              Cargando inventario...
            </p>
          </div>
        ) : (
                ...
                <div className="space-y-2 pl-2">
                  {groupedInventory[categoria].map((item: any) => (
                    ...
                    >
                      {/* Icono Caja */}
                      <div
                        className={`p-2 rounded-lg ${selectedItem?.id === item.id ? "bg-white text-[#719c44]" : "bg-[#f2f5f0] text-[#817e7e]"}`}
                      >
                        <Box size={18} />
                      </div>
                      ...
                      <div className="flex-1 min-w-0">
                        <p
                          className={`font-bold text-sm truncate ${selectedItem?.id === item.id ? "text-[#353131]" : "text-[#817e7e]"}`}
                        >
                      ...
                      <span
                        className={`text-xs font-bold px-2 py-1 rounded-lg ${selectedItem?.id === item.id ? "bg-[#719c44] text-white" : "bg-[#e5e7eb] text-[#817e7e]"}`}
                      >
      ...
      <div className="lg:col-span-8 bg-[#f9fafb] p-6 rounded-2xl border border-dashed border-[#c0c6b6] h-[calc(100vh-140px)] flex flex-col overflow-hidden relative">
        {!selectedItem ? (
          <div className="flex flex-col items-center justify-center h-full text-[#817e7e] animate-in zoom-in-95 duration-300">
            ...
            <h3 className="text-lg font-bold text-[#353131]">
              Selecciona un producto
            </h3>
            <p className="text-sm">
              El sistema analizará el padrón para sugerirte la mejor IAP.
            </p>
          </div>
        ) : loadingMatch ? (
          <div className="flex flex-col items-center justify-center h-full text-[#719c44] animate-pulse">
            <div className="animate-spin rounded-full h-10 w-10 border-b-2 border-[#719c44] mb-4"></div>
            <p className="font-medium text-[#353131]">
              Analizando prioridades y necesidades...
            </p>
          </div>
          ...
          <div className="flex flex-col items-center justify-center h-full text-[#817e7e]">
            ...
            <p className="font-medium text-[#353131]">
              No se encontraron IAPs candidatas.
            </p>
            <p className="text-xs max-w-xs text-center mt-1">
              Verifica que existan instituciones Activas, Certificadas, con
              Donataria y Padrón.
            </p>
          </div>
        ) : (
            ...
            <div className="flex justify-between items-end mb-4 bg-white p-4 rounded-xl border border-[#c0c6b6]/30 shadow-sm">
              <div>
                <h3 className="text-lg font-bold text-[#353131] flex items-center gap-2">
                  ...
                  Sugerencias para:{" "}
                  <span className="text-[#719c44] uppercase">
                    {selectedItem.nombre_producto}
                  </span>
                </h3>
                <p className="text-xs text-[#817e7e] mt-1 flex items-center gap-1">
                  <Info size={12} /> Ordenado por: Clasificación &gt; Historial
                  (0 donativos) &gt; Necesidad
                </p>
              </div>
            </div>
            ...
            <div className="bg-white border border-[#c0c6b6]/30 rounded-xl overflow-hidden shadow-sm flex-1 overflow-y-auto custom-scrollbar">
              <table className="w-full text-left">
                <thead className="bg-[#f2f5f0] text-[#353131] text-xs uppercase font-bold sticky top-0 z-10 shadow-sm">
                  <tr>
                    <th className="p-4 w-16 text-center text-[#719c44]">
                      Rank
                    </th>
                ...
                <tbody className="divide-y divide-[#f2f5f0]">
                  {sugerencias.map((iap, index) => (
                    <tr
                      key={iap.id}
                      className={`group hover:bg-[#f2f5f0] transition-colors ${index === 0 ? "bg-[#fffff0]" : ""}`}
                    >
                      ...
                      <td className="p-4">
                        <p className="font-bold text-[#353131] text-sm">
                          {iap.nombre_iap}
                        </p>
                        <p className="text-[10px] text-[#817e7e] mt-0.5 uppercase">
                          Nec. Primaria:{" "}
                          <span className="font-medium text-[#719c44]">
                            {iap.necesidad_primaria || "N/A"}
                          </span>
                        </p>
                      </td>
                      <td className="p-4 text-center">
                        <span
                          className={`px-2.5 py-1 rounded-md text-[10px] font-bold border ${getBadgeColor(iap.clasificacion)}`}
                        >
                      ...
                      <td className="p-4">
                        <div className="flex flex-wrap gap-1">
                          {iap.razones.map((razon: string, i: number) => (
                            <span
                              key={i}
                              className="text-[10px] text-[#5e6e52] flex items-center gap-1 bg-[#f9fafb] border border-[#c0c6b6] px-2 py-0.5 rounded-full"
                            >
                              <ShieldCheck
                                size={10}
                                className="text-[#719c44]"
                              />
                              {razon}
                            </span>
                          ))}
        ...
        >
          <div className="space-y-6 p-2">
            <div className="bg-[#f2f5f0] p-5 rounded-xl border border-[#c0c6b6] flex justify-between items-center shadow-sm">
              <div>
                <p className="text-xs text-[#719c44] font-bold uppercase mb-1 flex items-center gap-1">
                  <ShieldCheck size={12} /> Se entregará a:
                </p>
                <p className="text-lg font-bold text-[#353131] leading-tight">
                  {iapSeleccionada?.nombre_iap}
                </p>
              </div>
              <div className="text-right">
                <p className="text-xs text-[#719c44] font-bold uppercase mb-1">
                  Producto:
                </p>
                <p className="text-sm font-bold text-[#353131]">
                  {selectedItem?.nombre_producto}
                </p>
              </div>
            </div>
            ...
            <div className="bg-white p-4 rounded-xl border border-[#e5e7eb]">
              <label className="block text-sm font-bold text-[#353131] mb-2">
                Cantidad a Entregar ({selectedItem?.unidad_medida})
              </label>
              <div className="flex items-center gap-4">
                ...
                <div className="text-right text-xs text-[#817e7e] font-medium min-w-[80px]">
                  Disponible:
                  <br />
                  <span className="text-lg font-bold text-[#719c44]">
                    {selectedItem?.stock_actual ?? selectedItem?.cantidad}
                  </span>
                </div>
              </div>
            </div>
