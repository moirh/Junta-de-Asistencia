  ...
  return (
    <>
      ...
      <div className="w-full overflow-x-hidden bg-gray-50 text-gray-900 min-h-screen pb-10">
        <div className="w-full overflow-hidden"> <Slider /> </div>
        ...
        <div className="w-full px-4 md:px-6 lg:px-10 py-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            ...
            <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 flex items-center gap-4">
              <div className="p-3 bg-blue-100 text-blue-600 rounded-full"><LayoutDashboard className="w-6 h-6" /></div>
              <div>
                <p className="text-gray-500 text-sm">Total Acuerdos</p>
                <h3 className="text-2xl font-bold text-gray-800">{totalAcuerdos}</h3>
              </div>
            </div>

            {/* Widget Pendientes */}
            <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 flex items-center gap-4">
              <div className="p-3 bg-yellow-100 text-yellow-600 rounded-full"><ListTodo className="w-6 h-6" /></div>
              <div>
                <p className="text-gray-500 text-sm">Pendientes</p>
                <h3 className="text-2xl font-bold text-gray-800">{recordatoriosPendientes}</h3>
              </div>
            </div>
            ...
            <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 flex items-center gap-4">
              ...
              <div className="overflow-hidden">
                ...
                <h3 className="text-lg font-bold text-gray-800 truncate whitespace-nowrap" title={proximoTexto}>
                  {proximoTexto}
                </h3>
              </div>
            </div>
          ...
          <div className="flex flex-wrap gap-6 items-stretch w-full">
            ...
            <div className="flex-1 min-w-[350px] bg-white rounded-2xl shadow-md p-6 border border-gray-200 flex flex-col">
              <div className="flex justify-between items-center mb-4">
                ...
                <button onClick={() => openModal("acuerdo")} className="p-2 bg-blue-600 text-black rounded-full hover:bg-blue-700 transition shadow-md">
                  <Plus className="w-4 h-4" />
                </button>
              </div>
              <div className="space-y-4 flex-1 max-h-[500px] overflow-y-auto">
                {loading ? <p>Cargando...</p> : acuerdos.map((a) => (
                    <AgreementCard key={a.id} agreement={a} onDelete={handleDeleteAcuerdo} onToggle={() => handleToggleAcuerdo(a.id)} />
                  ))
                }
                {!loading && acuerdos.length === 0 && <p className="text-gray-400 text-center">Sin acuerdos.</p>}
              </div>
            </div>
            ...
            <div className="flex-1 min-w-[350px] bg-white rounded-2xl shadow-md p-6 border border-gray-200 flex flex-col">
              <div className="flex justify-between items-center mb-4">
                <h2 className="text-lg font-semibold text-gray-800">Recordatorios & Calendario</h2>
                <button onClick={() => openModal("recordatorio")} className="p-2 bg-yellow-500 text-black rounded-full hover:bg-yellow-600 transition shadow-md">
                  <Plus className="w-4 h-4" />
                </button>
              </div>
              ...
              <div className="grid md:grid-cols-2 gap-4 flex-1">
                <div className="space-y-4 max-h-[400px] overflow-y-auto">
                  {loading ? <p>Cargando...</p> : reminders.map((r) => (
                      <ReminderCard key={r.id} reminder={r} onToggle={() => handleToggleReminder(r.id, r.done)} onDelete={handleDeleteRecordatorio} />
                    ))
                  }
                  {!loading && reminders.length === 0 && <p className="text-gray-400 text-center">Sin recordatorios.</p>}
                </div>
      ...
      {/* --- MODAL --- */}
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={modalType === "acuerdo" ? "Nuevo Acuerdo" : "Nuevo Recordatorio"}>
        <form onSubmit={handleSubmit} className="space-y-5">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Título</label>
            <input type="text" required placeholder="Escribe un título..." className="w-full p-2.5 bg-gray-50 border border-gray-300 rounded-lg outline-none text-gray-900" value={formData.title} onChange={(e) => setFormData({ ...formData, title: e.target.value })} />
          </div>
          {modalType === "acuerdo" && (
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Descripción</label>
              <textarea required rows={3} placeholder="Detalles..." className="w-full p-2.5 bg-gray-50 border border-gray-300 rounded-lg outline-none text-gray-900" value={formData.description} onChange={(e) => setFormData({ ...formData, description: e.target.value })} />
            </div>
          )}
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Fecha</label>
            <input type="date" required className="w-full p-2.5 bg-gray-50 border border-gray-300 rounded-lg outline-none text-gray-900 [color-scheme:light]" value={formData.date} onChange={(e) => setFormData({ ...formData, date: e.target.value })} />
          </div>
          <div className="pt-2">
            <button type="submit" className="w-full bg-blue-600 text-black py-2.5 rounded-lg hover:bg-blue-700 transition font-medium shadow-sm">Guardar</button>
          </div>
        </form>
      </Modal>
    </>
