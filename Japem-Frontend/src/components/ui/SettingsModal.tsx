    ...
    try {
      ...
      if (profileData.currentPass && profileData.newPass) {
        await changePassword({
          current_password: profileData.currentPass,
          new_password: profileData.newPass,
        });
  ...
  const handleSaveUser = async () => {
    ...
    try {
      if (editingId) {
        ...
        setUsers(
          users.map((u) => (u.id === editingId ? updated.user || updated : u)),
        );
        Swal.fire({
      ...
      Swal.fire({
        title: "Error",
        text: e.response?.data?.message || "Error al procesar.",
        icon: "error",
        confirmButtonColor: "#353131",
      });
    } finally {
        ...
        : roleName === "admin"
          ...
          : roleName === "editor"
            ...
            : roleName === "donativos"
              ...
              : roleName === "asistencial"
                ? "bg-rose-100 text-rose-700 border-rose-200"
                : "bg-gray-100 text-gray-600 border-gray-200";
    return (
      <span
        className={`px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase border ${style}`}
      >
        {roleName}
      </span>
    );
    ...
    >
      <div className="flex flex-col lg:flex-row h-[600px] bg-white rounded-b-xl overflow-hidden">
        ...
        <div className="lg:w-64 bg-gray-50 border-r border-gray-200 p-5 flex flex-col gap-2">
          <p className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-2 ml-2">
            Cuenta
          </p>
          <button
          ...
          {canViewUsers && (
            <>
              <p className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-2 mt-4 ml-2">
                Administración
              </p>
              <button
                ...
                <div className="flex items-center gap-6 pb-6 border-b border-gray-100">
                  <div className="relative group cursor-pointer">
                    <div className="w-24 h-24 rounded-full p-1 border-2 border-dashed border-[#719c44]/50 group-hover:border-[#719c44] transition-all">
                      <img
                        src={`https://ui-avatars.com/api/?name=${profileData.name || "User"}&background=719c44&color=fff&size=128`}
                        alt="Avatar"
                  ...
                  <div>
                    ...
                    <div className="mt-3">{getRoleBadge(profileData.role)}</div>
                  </div>
                </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-2">
                      ...
                      <div className="relative group">
                        <User
                          className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 group-focus-within:text-[#719c44] transition-colors"
                          size={18}
                        />
                        <input
                          type="text"
                ...
                <div className="flex justify-between items-center mb-6">
                  <div>
                    ...
                    <p className="text-sm text-gray-500">
                      Administra los accesos a la plataforma.
                    </p>
                  </div>
                  {canCreateUser && !showUserForm && (
                    <button
                      onClick={() => setShowUserForm(true)}
                      className="cursor-pointer bg-[#353131] hover:bg-black text-white px-4 py-2.5 rounded-xl font-bold text-sm flex items-center gap-2 shadow-md transition-all active:scale-95"
                    >
                  ...
                  <div className="bg-white p-6 rounded-2xl border border-gray-200 shadow-xl mb-6 relative animate-in slide-in-from-top-4 ring-1 ring-black/5">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mb-6">
                      <div className="space-y-1">
                        ...
                        <input
                          type="text"
                          className="w-full p-3 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:bg-white focus:border-[#719c44] outline-none transition-all"
                          value={newUser.name}
                          onChange={(e) =>
                      ...
                      <div className="md:col-span-2 space-y-1">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                          {[
                            ...
                            <label
                              key={r}
                              className={`cursor-pointer border rounded-xl p-3 text-center text-sm font-medium transition-all ${newUser.role === r ? "border-[#719c44] bg-[#f2f5f0] text-[#719c44]" : "border-gray-200 hover:border-gray-300"}`}
                            >
                              <input
                                type="radio"
                                name="role"
                    ...
                    <div className="flex justify-end gap-3">
                      <button
                        onClick={cancelForm}
                        className="cursor-pointer px-5 py-2.5 text-gray-500 font-bold hover:bg-gray-100 rounded-xl transition-all"
                      >
                        Cancelar
                      </button>
                      <button
                        ...
                        disabled={loading}
                        className="cursor-pointer bg-[#719c44] hover:bg-[#5e8239] text-white px-6 py-2.5 rounded-xl font-bold shadow-lg shadow-[#719c44]/30 transition-all active:scale-95"
                      >
                        {loading
                          ? "Guardando..."
                          : editingId
                  ...
                  <div className="flex-1 overflow-y-auto custom-scrollbar divide-y divide-gray-100">
                    {users.map((u) => (
                      ...
                      >
                        ...
                        <div className="col-span-4">{getRoleBadge(u.role)}</div>
                        <div className="col-span-3 flex justify-end gap-2 opacity-60 group-hover:opacity-100 transition-opacity">
                          <button
                            onClick={() => startEditing(u)}
                            className="cursor-pointer p-2 text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-all"
                            title="Editar"
                          >
                            <Edit2 size={16} />
                          </button>
